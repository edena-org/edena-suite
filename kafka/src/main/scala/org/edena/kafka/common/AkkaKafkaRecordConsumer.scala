...
private class AkkaKafkaRecordConsumer[K, V: Format](
  topics: Seq[String],
  ...
  ec: ExecutionContext
) extends KafkaAsyncRecordConsumer[K, V] with ClientConfig {
...
object AkkaKafkaRecordConsumer {
  ...
  ): KafkaAsyncRecordConsumer[K, V] =
    new AkkaKafkaRecordConsumer[K, V](topics, dlqTopic, parallelism, partitionParallelism, keyDeserializer, keySerializer, config)
  ...
  def ofStringKey[V: Format](
    ...
    implicit system: ActorSystem, ec: ExecutionContext
  ): KafkaAsyncRecordConsumer[String, V] =
  ...
  def ofUUIDKey[V: Format](
    ...
    implicit system: ActorSystem, ec: ExecutionContext
  ): KafkaAsyncRecordConsumer[java.util.UUID, V] =
