...
import org.edena.ada.server.dataaccess.dataset.DataViewStore
...
import org.edena.ada.server.models.{DataView, WidgetGenerationMethod, WidgetSpec}
...
protected[controllers] class DataViewControllerImpl @Inject() (
  ...
  ) extends AdaCrudControllerImpl[DataView, BSONObjectID](dsaf.applySync(dataSetId).get.dataViewStore)
  ...
  protected lazy val dataViewRepo = dsa.dataViewStore
  protected lazy val fieldRepo = dsa.fieldStore
  ...
  private implicit val sortTypeFormatter = EnumFormatter(SortType)
  ...
  override protected[controllers] val form = Form(
    mapping(
      ...
      "initTableSortFieldName" -> optional(nonEmptyText),
      "initTableSortType" -> optional(of[SortType.Value]),
    ...
    ((item: DataView) => Some((item._id, item.name, item.filterOrIds, item.tableColumnNames, item.initTableSortFieldName, item.initTableSortType, item.widgetSpecs, item.elementGridWidth, item.default, item.isPrivate, item.generationMethod)))
  )
  ...
  override protected def getFormEditViewData(
    ...
    val setCreatedByFuture =
      form.value match {
        case Some(dataView) => DataViewStore.setCreatedBy(userRepo, Seq(dataView))
        case None => Future(())
      }
...
//              criteria = Seq(FilterIdentity.name #-> filterIds),
  ...
  override protected def getListViewData(
    ...
    val setCreatedByFuture = DataViewStore.setCreatedBy(userRepo, page.items)
  ...
  override protected def updateCall(
    ...
    for {
      existingDataViewOption <- store.get(dataView._id.get)
