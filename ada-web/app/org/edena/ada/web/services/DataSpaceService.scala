...
import org.edena.ada.server.models.{DataSetMetaInfo, DataSpaceMetaInfo, User}
import org.edena.ada.web.models.security.DeadboltUser
import org.edena.core.store.Criterion.Infix
import org.edena.play.security.{ActionSecurity, HasCurrentUser, SecurityRole}
...
class DataSpaceServiceImpl @Inject() (
  ...
  ) extends DataSpaceService with HasCurrentUser {
  ...
  override def getDataSpaceForCurrentUser(
    ...
    for {
      ...
      foundChildren <- dataSpaceMetaInfoRepo.find("parentId" #== dataSpace._id)
    } yield
      user.map { user =>
  ...
  override def allAsTree: Future[Traversable[DataSpaceMetaInfo]] = {
    dataSpaceMetaInfoRepo.find().map { dataSpaces =>
      val idDataSpaceMap = dataSpaces.map( dataSpace =>
        try {
          val id = dataSpace._id.get
          (id, dataSpace)
        } catch {
          case e: Exception =>
            println("Error: " + e)
