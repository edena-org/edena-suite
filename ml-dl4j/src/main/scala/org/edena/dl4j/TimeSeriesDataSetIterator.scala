...
private class TimeSeriesDataSetIterator(
  ...
  private def mdsToDataSet(mds: MultiDataSet): DataSet = {
    val f: INDArray = getOrNull(mds.getFeatures, 0)
    val fm: INDArray = getOrNull(mds.getFeaturesMaskArrays, 0)
...
object TimeSeriesDataSetIterator {
  ...
  def apply(
    ...
    val (inputReader, labelReader) = CsvFeaturesAndLabelReader(
      featureBaseDir, outputBaseDir, startIndex, endIndex, featuresSkipLines
    )

    apply(batchSize, outputSize, inputReader, labelReader)
  }
  ...
  def apply(
    ...
    val (trainingInputReader, trainingLabelReader) = CsvFeaturesAndLabelReader(
      featureBaseDir, outputBaseDir, trainingIndeces, featuresSkipLines
    )

    val trainingSet = apply(batchSize, outputSize, trainingInputReader, trainingLabelReader)

    val (validationInputReader, validationLabelReader) = CsvFeaturesAndLabelReader(
      featureBaseDir, outputBaseDir, validationIndeces, featuresSkipLines
    )

    val validationSet = apply(batchSize, outputSize, validationInputReader, validationLabelReader)
