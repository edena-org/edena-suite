...
object BasicStatsCalc extends Calculator[BasicStatsCalcTypePack] {
  ...
  override def fun(o: Unit) = { values: Traversable[IN] =>
    ...
    if (flattenedValues.nonEmpty) {
      ...
      val variance = Math.max(sqSum / count - mean * mean, 0)  // variance cannot be negative... just to fix a potential rounding error
  ...
  override def postFlow(o: Unit) = { accum: INTER =>
    if (accum.count > 0) {
      ...
      val variance = Math.max((accum.sqSum / accum.count) - mean * mean, 0) // variance cannot be negative... just to fix a potential rounding error
