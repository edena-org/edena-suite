...
class StreamFieldTypeInferrerTest extends AsyncFlatSpec with Matchers {
  ...
  "Enum field type" should "be inferred only for enum values" in {
    ...
    for {
      _ <- shouldBeEnumType(Map(0l -> "Male")) (Seq("Male", " Male"))
      _ <- shouldBeEnumType(Map(0l -> "Both", 1l -> "Female", 2l -> "Male")) (Seq(" Male", "Female", "Male ", "na", "Both", "Female"))
      _ <- shouldBeEnumType(Map(0l -> "1", 1l -> "Sun")) (Seq("1", " 1", "1", "null", "Sun "))
    } yield
      succeed
  }
