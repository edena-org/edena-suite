...
class FieldTypeInferrerTest extends FlatSpec with Matchers {
  ...
  "Enum field type" should "be inferred only for enum values" in {
    def shouldBeEnumType(enumValues: Map[Long, String]) = shouldBeInferredType(FieldTypeSpec(FieldTypeId.Enum, false, enumValues))_

    shouldBeEnumType(Map(0l -> "Male")) (Seq("Male", " Male"))
    shouldBeEnumType(Map(0l -> "Both", 1l -> "Female", 2l -> "Male")) (Seq.fill(1000)(Seq(" Male", "Female", "Male ", "na", null, "Both", "Female")).flatten)
    shouldBeEnumType(Map(0l -> "1", 1l -> "Sun")) (Seq("1", " 1", "1", "null", "Sun "))
  }
  ...
  "Enum array field type" should "be inferred only for enum array values" in {
    def shouldBeEnumArrayType(enumValues: Map[Long, String]) = shouldBeInferredType(FieldTypeSpec(FieldTypeId.Enum, true, enumValues))_

    shouldBeEnumArrayType(Map(0l -> "Male")) (Seq(
