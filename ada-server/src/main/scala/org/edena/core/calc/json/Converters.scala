...
class ArrayGroupUniqueLongCountsConverter extends ArrayGroupLongConverter[Any] {
  override def specificUseClass = Some(classOf[GroupUniqueDistributionCountsCalc[Any, Long]])
}
...
private[json] abstract class ArrayGroupLongConverter[G: TypeTag] extends JsonInputConverter[Array[(Option[G], Option[Long])]] {

  override def apply(fields: Seq[Field]) = {
    val groupConverter = jsonToValue[G](fields(0))
    val longConverter = jsonToArrayNumericValue[Long](fields(1))

    (jsObject: JsObject) =>
      val group = groupConverter(jsObject)
      val array = longConverter(jsObject)
      array.map((group, _))
  }

  override def inputType = typeOf[Array[(Option[G], Option[Long])]]
}
