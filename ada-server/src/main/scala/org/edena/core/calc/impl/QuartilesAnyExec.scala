...
import org.edena.ada.server.calc._
import org.edena.ada.server.field.FieldUtil.{fieldTypeOrdering, valueOrdering}
import org.edena.core.calc.{CalculatorTypePack, NoOptionsCalculatorTypePack}
...
private trait QuartilesAnyExec[F] extends CalculatorExecutor[QuartilesCalcNoOptionsTypePack[Any], F] with DispatchHelper[F, QuartilesCalcTypePack] {
  ...
  override def execJsonRepoStreamed(
    ...
    implicit actorSystem: ActorSystem, materializer: Materializer
      ...
      (executor) => executor.execJsonRepoStreamed(flowOptions, _, withProjection, toFields(fields))(dataRepo, criterion)
    )(fields, Future(None))
  ...
  override def execJsonRepoStreamedA(
    ...
    implicit actorSystem: ActorSystem, materializer: Materializer
      ...
      (executor) => executor.execJsonRepoStreamedA(flowOptions, _, withProjection, scalarOrArrayField, toFields(fields))(dataRepo, criterion)
    )(fields, Future(None))
