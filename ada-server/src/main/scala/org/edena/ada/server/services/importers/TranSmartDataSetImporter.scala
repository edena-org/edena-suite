...
import org.edena.ada.server.dataaccess.dataset.CategoryStore._
import org.edena.ada.server.field.{FieldType, FieldTypeHelper}
import org.edena.ada.server.dataaccess.StoreTypes.{CategoryStore, FieldStore}
    ...
    for {
      ...
      _ <- {
        if (importInfo.mappingPath.isDefined) {
          ...
          dataSetService.updateFields(dsa.fieldStore, fields, true, true)
        }
      }
