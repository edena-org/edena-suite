...
import org.edena.ada.server.models.redcap.{Metadata, FieldType => RCFieldType}
...
private class RedCapDataSetImporter @Inject() (
  ...
  override def runAsFuture(importInfo: RedCapDataSetImport): Future[Unit] = {
    ...
    for {
      ...
      categoryRepo = dsa.categoryStore
      ...
      categoryIdsToInherit <-
        if (importInfo.categoriesToInheritFromFirstVisit.nonEmpty)
          categoryRepo.find("name" #-> importInfo.categoriesToInheritFromFirstVisit).map(_.map(_._id.get))
        else
          Future(Nil)
...
//        val fieldType = getFieldType(fieldName)
    ...
    def inferDictionary(
      ...
      metadatas.filter { metadata =>
        ...
        if (metadata.field_type != RCFieldType.checkbox) {
          ...
          val field = Field(fieldName, Some(metadata.field_label), fieldTypeSpec.fieldType, fieldTypeSpec.isArray, stringEnumValues, None, None, None, None, Nil, categoryId)
    ...
    for {
      ...
      categoryNameIds <- Future.sequence {
        metadatas.map(_.form_name).toSet.map { categoryName: String =>
          categoryRepo.find("name" #== categoryName).flatMap { categories =>
  ...
  private def getEnumValues(metadata: Metadata): Map[Long, String] = {
