...
import org.edena.spark_ml.transformers._
import org.edena.spark_ml.models.VectorScalerType
import org.edena.spark_ml._
...
import org.edena.spark_ml.models.result.{ClassificationResultsHolder, RegressionResultsHolder}
import org.edena.spark_ml.models.setting.{ClassificationLearningSetting, RegressionLearningSetting, TemporalClassificationLearningSetting, TemporalRegressionLearningSetting}
...
private[services] class MachineLearningServiceImpl @Inject() (
  sparkApp: SparkApp,
  ...
  val rcStatesWindowFactory: RCStatesWindowFactory
   ...
   override val setting = SparkMLServiceSetting(
    repetitionParallelism = configuration.optionalInt("ml.repetition_parallelism"),
    binaryClassifierInputName = configuration.optionalString("ml.binary_classifier.input"),
    useConsecutiveOrderForDL = configuration.optionalBoolean("ml.dl_use_consecutive_order_transformers"),
    debugMode = configuration.optionalBoolean("ml.debug_mode").getOrElse(false)
  )
