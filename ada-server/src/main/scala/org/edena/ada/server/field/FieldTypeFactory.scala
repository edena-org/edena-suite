...
import org.edena.ada.server.dataaccess.ConversionUtil._
...
private case class EnumFieldType(
    ...
    enumValueMap: Map[Long, String]
  ) extends FormatFieldType[Long] {
  ...
  override protected def valueToDisplayStringNonEmpty(value: Long) =
  ...
  override protected def jsonToValueWoNull(json: JsReadable): Long =
    json match {
      case JsNumber(number) =>
        try {
          val long = number.toLongExact
          if (enumValueMap.contains(long))
            long
          else
  ...
  override protected def valueToJsonNonEmpty(value: Long): JsValue =
