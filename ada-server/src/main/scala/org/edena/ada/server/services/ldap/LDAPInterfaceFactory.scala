...
object LDAPInterfaceFactory {
  ...
  def createLocalServer(
    ...
    actorSystem.registerOnTermination(terminateInterface(server))
  ...
  def createConnectionPool(
    ...
    if (result == ResultCode.SUCCESS) {
      ...
      Some(
        createConnectionPool(connection, processor, actorSystem)
      )
    } else {
  ...
  private def createConnectionPool(
    ...
    // hook interface in lifecycle for proper cleanup
    actorSystem.registerOnTermination(terminateInterface(connectionPool))
