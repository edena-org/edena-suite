...
import org.edena.ada.server.dataaccess.ConversionUtil
import org.edena.ada.server.models.egait.{EGaitKineticData, SpatialPoint}
import org.apache.commons.codec.binary.Base64
import org.edena.core.util.ZipFileIterator
...
//      .setAcceptAnyCertificate(true)
  ...
  private def confValue(key: String) = configuration.getString(key)
  ...
  override def getProxySessionToken(certificateFileName: String): Future[String] = {
    ...
    val request = ws.url(baseUrl + Url.Session).withHttpHeaders(
  ...
  override def getConnectionToken(
    ...
    val request = ws.url(baseUrl + Url.ServiceConnectionToken + serviceName).withHttpHeaders(
    ...
    for {
      ...
      userSessionId <- {
        ...
        request.post(loginInfoXML).map { response =>
          (response.body[scala.xml.Elem] \\ "SessionId").text
        }
      }
    } yield
      userSessionId
    ...
    for {
      ...
      searchSessionIds <- {
        ...
        request.post(findAllFilterXML).map( response =>
          (response.body[scala.xml.Elem] \\ "Session" \ "SessionId").map(_.text)
        )
      }
    } yield
      searchSessionIds
...
//      def intValue(startIndex: Int) = ((timePointData(startIndex + 1) & 0xff) << 8) | (timePointData(startIndex) & 0xff)
  ...
  private def withXmlContent(request: StandaloneWSRequest): StandaloneWSRequest =
    request.withHttpHeaders("Content-Type" -> "application/xml")

  private def withConnectionToken(connectionToken: String)(request: StandaloneWSRequest): StandaloneWSRequest =
    request.withHttpHeaders("connect-token" -> connectionToken)

  private def withUserSessionId(userSessionId: String)(request: StandaloneWSRequest): StandaloneWSRequest =
    request.withHttpHeaders("user-session" -> userSessionId)

  private def withRequestTimeout(timeout: Duration)(request: StandaloneWSRequest): StandaloneWSRequest =
