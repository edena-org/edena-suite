...
import org.edena.ada.server.models.{LdapUser, UserGroup}
...
protected class LdapServiceImpl @Inject()(
  ...
  private def setupInterface: Option[LDAPInterface] =
    settings.mode match {
      ...
      case "local" =>
        Some(
          LDAPInterfaceFactory.createLocalServer(settings.dit, settings.port, actorSystem)
        )
      ...
      case "remote" =>
        ...
        LDAPInterfaceFactory.createConnectionPool(
          settings.host, settings.port, settings.encryption, settings.trustStore, settings.bindDN, password, actorSystem,
          connectionOptions
        )
