...
import org.edena.ada.server.util.DistinctValueHelper
import org.edena.store.json.JsonReadonlyStoreExtra._
import org.edena.core.store.{And, AscSort, Criterion, DescSort, NotEqualsNullCriterion}
...
private[services] class StatsServiceImpl @Inject() (
  config: Config)(
  implicit val system: ActorSystem, materializer: Materializer
) extends StatsService
  with OneWayAnovaHelper
  with ChiSquareHelper[Int, Option[Any]]
  with DistinctValueHelper {

  private implicit val ftf = FieldTypeHelper.fieldTypeFactory()
  private val defaultNumericBinCount = 20

  private val maxDistinctCount = config.optionalInt("stats.distMaxCollectDistinctCount").getOrElse(20)
  ...
  override def calcUniqueDistributionCountsFromRepo(
    ...
    // aux function to calc unique dist. count by collecting distinct values
    def calcUniqueDistByDistinctValuesAux[T: Reads] =
      for {
        distinctValues <- collectDistinctValues[T](
          dataRepo, field.name, criterion, maxValueCount = Some(maxDistinctCount)
        )

        uniqueCounts <- calcUniqueCountsFromRepo(
          field.name, distinctValues, dataRepo, criterion
        )
      } yield
        uniqueCounts
    ...
    field.fieldType match {
      ...
      case FieldTypeId.Enum =>
        ...
        calcUniqueCountsFromRepo(field.name, values, dataRepo, criterion)
      ...
      case FieldTypeId.String =>
        calcUniqueDistByDistinctValuesAux[String]

      case FieldTypeId.Integer =>
        calcUniqueDistByDistinctValuesAux[Long]

      case _ =>
        for {
          jsons <- dataRepo.find(criterion = criterion, projection = Seq(field.name))
        } yield {
  ...
  private def calcUniqueCountsFromRepo[T](
    ...
    val countFutures = values.par.map { value =>
      val finalCriteria = criterion AND (fieldName #== value)
    ...
    val findNoneCriteria = criterion AND (fieldName #=@)
  ...
  override def calcGroupedUniqueDistributionCountsFromRepo(
    ...
    for {
      groupValues <- groupValues(dataRepo, criterion, groupField)

      seriesCounts <- {
        val groupFieldName = groupField.name
        val countFutures = groupValues.par.map { value =>
          val finalCriteria = criterion AND (groupFieldName #== value)
        ...
        val undefinedGroupCriteria = criterion AND (groupFieldName #=@)
  ...
  override def calcNumericDistributionCountsFromRepo(
    ...
    createRepoCountSpec(field, numericBinCountOption).map( repoCountSpec =>
      calcNumericalCountsFromRepo(dataRepo, criterion)(repoCountSpec)
    ).getOrElse(
      Future(Nil)
    )
  ...
  override def calcGroupedNumericDistributionCountsFromRepo(
    ...
    for {
      groupVals <- groupValues(dataRepo, criterion, groupField)
  ...
  private def groupValues(
    ...
    groupFieldSpec.fieldType match {
      ...
      case _ =>
        for {
          jsons <- dataRepo.find(
            criterion = criterion AND (groupField.name #!@),
  ...
  private def calcNumericalCountsFromRepo[T](
    ...
    for {
      // get min and max
      (minOption, maxOption) <- getMinMaxBigDecimal(dataRepo, criterion)(spec)
      ...
      binnedCounts <-
        minOption.zip(columnCountStepSizeOption).headOption.map { case (min, (columnCount, stepSize)) =>
          val rangeStartAndCriterion = calcRangeStartAndCriteria(spec, min, stepSize, columnCount)

          val futures = rangeStartAndCriterion.par.map {
            case (start, rangeCriterion) => dataRepo.count(rangeCriterion AND criterion).map((start, _))
          }
  ...
  override def calcSeqNumericDistributionCountsFromRepo(
    ...
    calcSeqNumericalCountsFromRepo(dataRepo, criterion)(repoCountSpecs)
  }
  ...
  private def calcSeqNumericalCountsFromRepo(
    ...
    val specMinMaxFutures = Future.sequence(
      specs.map(spec =>
        getMinMaxBigDecimal(dataRepo, And(criterion +: notNullCriteria))(spec).map(minMax => (spec, minMax))
      )
    )
      ...
      binnedCounts <- {
        ...
        if (definedSpecMinColumnCountStepSizes.size == specMinColumnCountStepSizes.size) {
          ...
          val futures = rangeStartAndCriteria.par.map { rangeStartAndCriteria =>
            ...
            val rangeCriteria = rangeStartAndCriteriaSeq.map(_._2)
            val starts = rangeStartAndCriteriaSeq.map(_._1)
            dataRepo.count(And(criterion +: rangeCriteria)).map((starts, _))
          }
  ...
  private def getMinMaxBigDecimal[T](
    ...
    val maxFuture =
      if (spec.explMax.isDefined)
        ...
        dataRepo.max(spec.fieldName, criterion, true).map(
    ...
    val minFuture =
      if (spec.explMin.isDefined)
        ...
        dataRepo.min(spec.fieldName, criterion, true).map(
          _.flatMap(jsonToBigDecimalValue)
        )
  ...
  private def calcRangeStartAndCriteria[T](
    ...
    stepSize: BigDecimal,
    columnCount: Int
  ): Seq[(BigDecimal, Criterion)] =
    if (stepSize == 0) {
      val rangeCriterion = spec.fieldName #== spec.toRangeVal(min)
      Seq((min, rangeCriterion))
    } else {
  ...
  private def calcRangeStartAndCriteriaAux(
    ...
    val criteria = if (columnIndex < columnCount - 1)
      (fieldName #>= startVal) AND (fieldName #< endVal)
    else
      (fieldName #>= startVal) AND (fieldName #<= endVal)
    (start, criteria)
  }
  ...
  override def calcQuartilesFromRepo(
    ...
    def quartiles[T: Ordering](toDouble: T => Double) =
      calcQuartilesFromRepo[T](dataRepo, criterion, field, toDouble).map(
  ...
  private def calcQuartilesFromRepo[T: Ordering](
    ...
    for {
      // total length
      length <- dataRepo.count(criterion AND (field.name #!@))

      // create quartiles
      quants <-
        if (length > 0)
          createQuantilesAux(toDouble, length, dataRepo, criterion, field)
        else
          Future(None)
    } yield
      quants
...
//        fromValue.map { minValue => Seq(field.name #> minValue) }
      ...
      dataRepo.find(
        criterion = criterion AND (field.name #!@),
      ...
      lowerQuantileLessEqualsCountOption <-
        lowerQuantileOption match {
          case Some(lowerQuantile) =>
            dataRepo.count(
              criterion = criterion AND (field.name #<= lowerQuantile)
            ).map(Some(_))
...
//      medianOption <- getItem(length / 2, true)
        ...
        val optionFuture = for {
          ...
          val medianFuture =
            if (relativeMedianPos > 0) {
              dataRepo.find(
                criterion = criterion AND (field.name #> lower),
                projection = Seq(field.name),
          ...
          val lowerWhiskerFuture =
            dataRepo.find(
              criterion = criterion AND (field.name #>= lowerWhiskerValue),
              projection = Seq(field.name),
              sort = Seq(AscSort(field.name)),
              limit = Some(1)
            ).flatMap { results =>
          ...
          val upperWhiskerFuture =
            dataRepo.find(
              criterion = criterion AND (field.name #<= upperWhiskerValue),
              projection = Seq(field.name),
  ...
  override def getMinMax[T](
    ...
    // min and max futures
    val minFuture = dataRepo.min(field.name, criterion, true)
    val maxFuture = dataRepo.max(field.name, criterion, true)
  ...
  override def getNumericMinMax(
    ...
    def aux[T](toDouble: T => Double) =
      getMinMax[T](dataRepo, criterion, field).map {
...
//      result
  ...
  override def testIndependenceSorted(
    ...
    testIndependence(dataRepo, criterion, inputFields, targetField, keepUndefined).map { results =>
  ...
  override def testIndependence(
    ...
    val chiSquareResultsFuture =
      if (keepUndefined)
        chiSquareTestExec.execJsonRepoStreamed_(withProjection = true, Seq(targetField) ++ nonNumericInputFields)(dataRepo, criterion)
      else
        nullExcludedChiSquareTestExec.execJsonRepoStreamed_(withProjection = true, Seq(targetField) ++ nonNumericInputFields)(dataRepo, criterion)

    val anovaResultsFuture =
      if (keepUndefined)
        anovaTestExec.execJsonRepoStreamed_(withProjection = true, Seq(targetField) ++ numericInputFields)(dataRepo, criterion)
      else
        nullExcludedAnovaTestExec.execJsonRepoStreamed_(withProjection = true, Seq(targetField) ++ numericInputFields)(dataRepo, criterion)
  ...
  override def selectFeaturesAsAnovaChiSquare(
    ...
    testIndependenceSorted(dataRepo, criterion, inputFields, targetField, keepUndefined).map { results =>
  ...
  override def testAnovaForMultiCriteriaSorted(
    ...
    def calcStatsAux(
      ...
      dataRepo.find(
        criterion = criterion,
        projection = inputFields.map(_.name)
      ).map(
