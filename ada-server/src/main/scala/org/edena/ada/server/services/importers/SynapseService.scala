...
import org.edena.ada.server.models.synapse.JsonFormat._
import org.edena.ada.server.models.synapse._
import org.edena.ada.server.services.{AdaRestException, AdaUnauthorizedAccessRestException}
import org.edena.core.util.{ZipFileIterator, retry}
...
protected[services] class SynapseServiceWSImpl @Inject() (
  ...
  private val baseUrl = configuration.getString("synapse.api.rest.url")
  private val loginSubUrl = configuration.getString("synapse.api.login.url")
  private val prolongSessionUrl = configuration.getString("synapse.api.session.url")
  private val tableCsvDownloadStartSubUrl1 = configuration.getString("synapse.api.table_csv_download_start.url.part1")
  private val tableCsvDownloadStartSubUrl2 = configuration.getString("synapse.api.table_csv_download_start.url.part2")
  private val tableCsvDownloadResultSubUrl1 = configuration.getString("synapse.api.table_csv_download_result.url.part1")
  private val tableCsvDownloadResultSubUrl2 = configuration.getString("synapse.api.table_csv_download_result.url.part2")
  private val fileHandleSubUrl = configuration.getString("synapse.api.file_handle.url")
  private val fileDownloadSubUrl1 = configuration.getString("synapse.api.file_download.url.part1")
  private val fileDownloadSubUrl2 = configuration.getString("synapse.api.file_download.url.part2")
  private val columnModelsSubUrl1 = configuration.getString("synapse.api.table_column_models.url.part1")
  private val columnModelsSubUrl2 = configuration.getString("synapse.api.table_column_models.url.part2")
  private val columnFileHandleSubUrl1 = configuration.getString("synapse.api.column_file_handles.url.part1")
  private val columnFileHandleSubUrl2 = configuration.getString("synapse.api.column_file_handles.url.part2")
  private val fileColumnDownloadSubUrl1 = configuration.getString("synapse.api.file_column_download.url.part1")
  private val fileColumnDownloadSubUrl2 = configuration.getString("synapse.api.file_column_download.url.part2")
  private val fileColumnDownloadSubUrl3 = configuration.getString("synapse.api.file_column_download.url.part3")
  private val fileColumnDownloadSubUrl4 = configuration.getString("synapse.api.file_column_download.url.part4")
  private val fileColumnDownloadSubUrl5 = configuration.getString("synapse.api.file_column_download.url.part5")
  private val bulkDownloadStartUrl = configuration.getString("synapse.api.bulk_download_start.url")
  private val bulkDownloadResultUrl = configuration.getString("synapse.api.bulk_download_result.url")
  ...
  override def login: Future[Unit] = {
    ...
    request.post(data).map { response =>
      val newSessionToken = (response.body[JsValue].as[JsObject] \ "sessionToken").get.as[String]
  ...
  override def startCsvTableQuery(tableId: String, sql: String): Future[String] = {
    ...
    request.post(data).map { response =>
      handleErrorResponse(response)
      (response.body[JsValue].as[JsObject] \ "token").get.as[String]
    }
  }
  ...
  override def getFileHandle(fileHandleId: String): Future[FileHandle] = {
    ...
    request.get.map { response =>
      handleErrorResponse(response)
      response.body[JsValue].as[FileHandle]
    }
  }
  ...
  override def getTableColumnFileHandles(rowReferenceSet: RowReferenceSet): Future[TableFileHandleResults] = {
    ...
    request.post(Json.toJson(rowReferenceSet)).map { response =>
      handleErrorResponse(response)
      response.body[JsValue].as[TableFileHandleResults]
    }
  }
  ...
  private def handleErrorResponse(response: StandaloneWSResponse): Unit =
    response.status match {
  ...
  override def getTableColumnModels(tableId: String): Future[PaginatedColumnModels] = {
    ...
    getColumnModelsReq.get.map { response =>
      handleErrorResponse(response)
      response.body[JsValue].as[PaginatedColumnModels]
    }
  }
  ...
  def withSessionToken(request: StandaloneWSRequest): StandaloneWSRequest =
    request.withHttpHeaders("sessionToken" -> getSessionToken)

  def withJsonContent(request: StandaloneWSRequest): StandaloneWSRequest =
    request.withHttpHeaders("Content-Type" -> "application/json")

  def withRequestTimeout(timeout: Duration)(request: StandaloneWSRequest): StandaloneWSRequest =
