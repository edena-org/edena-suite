...
//    val fieldNameClassMap = createFieldNameClassMapFromDictionary(idFieldName, fieldNamesAndTypes)
    val cacheStoreFactory = new BinaryCacheCrudStoreFactory[ID](cacheName, ignite, repoFactory, getId, fieldNamesAndTypes.toMap)
    apply(
      cacheName,
      fieldNamesAndTypes,
  ...
  def apply[ID](
    ...
    cacheConfig.setCacheMode(CacheMode.REPLICATED) // LOCAL
