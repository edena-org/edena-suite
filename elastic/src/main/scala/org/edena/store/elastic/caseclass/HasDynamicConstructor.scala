...
import org.edena.core.util.{DynamicConstructor, DynamicConstructorFinder}
...
trait HasDynamicConstructor[E] {
  ...
  protected def constructorOrException(
    ...
    concreteClassName: Option[String]
  ): DynamicConstructor[E] = {
    val fieldNames = sourceMap.map(_._1).toSeq
    constructorOrException(fieldNames, concreteClassName)
  }
...
object TypeValueConverters {
  ...
  def enum[E <: Enumeration : TypeTag](enum: E) = (
    ru.typeOf[E#Value], (_: Any) match {
      ...
      case value: Any => throw new EdenaDataStoreException(s"Enum ${enum} expects a String value got ${value}.")
    }
  )
}